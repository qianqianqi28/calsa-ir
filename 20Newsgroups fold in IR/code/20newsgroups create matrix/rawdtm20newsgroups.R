...
training_folder <- "C:/Users/qi000005/OneDrive - Universiteit Utrecht/qi000005/paper 2/20Newsgroups fold in IR/data/20news-bydate-train/used for experiments/"
...
set.seed(123)
cho_train <- sample(nrow(training_dtm), size = 600)
training_dtm <- training_dtm[cho_train,]
...
testing_folder <- "C:/Users/qi000005/OneDrive - Universiteit Utrecht/qi000005/paper 2/20Newsgroups fold in IR/data/20news-bydate-test/used for experiments/"
...
cho_test <- sample(nrow(testing_dtm), size = 400)
testing_dtm <- testing_dtm[cho_test,]
...
for (i in 1:length(training_index)){
  if (grepl('comp.graphics', training_index[i])) {
    ...
    training_index[i] <- 3
  }else if (grepl('talk.politics.guns', training_index[i])) {
    training_index[i] <- 4
  }
}
...
for (i in 1:length(testing_index)){
  if (grepl('comp.graphics', testing_index[i])) {
    ...
    testing_index[i] <- 3
  } else if (grepl('talk.politics.guns', testing_index[i])) {
    testing_index[i] <- 4
  }
}
...
sum(testing_index==1)/sum(training_index==1)
sum(testing_index==2)/sum(training_index==2)
sum(testing_index==3)/sum(training_index==3)
sum(testing_index==4)/sum(training_index==4)
...
save(training_dtm, testing_dtm, training_index, testing_index, file = "C:\\Users\\qi000005\\OneDrive - Universiteit Utrecht\\qi000005\\paper 2\\20Newsgroups fold in IR\\created data\\created matrix\\rawdtm20newsgroups.Rdata")
