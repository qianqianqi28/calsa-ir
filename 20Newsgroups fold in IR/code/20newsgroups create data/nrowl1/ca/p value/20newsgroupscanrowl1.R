...
canrowl1_euc_20NG <- canrowl1_cos_20NG <- canrowl1_dot_20NG <- matrix(NA, nrow = length(p), ncol = length(dim))
...
for (i in 1:length(p)) {
  ...
  row.names(dtm.training.canrowl1.svd.ud)  <- training_index
  canrowl1_20NG <- meanprelsacapvalue(dtm.training.canrowl1.svd.ud, dtm.testing.canrowl1.svd.ud, training_index, testing_index, metric = dis_metric, dim, retrieved_document_number = nrow_dtm_training, recall_value)
  canrowl1_euc_20NG[i,] <- canrowl1_20NG[[1]]
  canrowl1_cos_20NG[i,] <- canrowl1_20NG[[2]]
  canrowl1_dot_20NG[i,] <- canrowl1_20NG[[3]]
  }

canrowl1_20NG_p <- p
canrowl1_20NG_dim <- dim

save(canrowl1_20NG_dim, canrowl1_20NG_p, canrowl1_euc_20NG,canrowl1_cos_20NG, canrowl1_dot_20NG, file='C:\\Users\\qi000005\\OneDrive - Universiteit Utrecht\\qi000005\\paper 2\\20Newsgroups fold in IR\\created data\\nrowl1\\ca\\p value\\meanprecanrowl120NG.Rdata')
