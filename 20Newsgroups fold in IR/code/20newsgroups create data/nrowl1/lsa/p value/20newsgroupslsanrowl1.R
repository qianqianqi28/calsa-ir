...
lsanrowl1_20NG <- list()
lsanrowl1_euc_20NG <- lsanrowl1_cos_20NG <- lsanrowl1_dot_20NG <- matrix(NA, nrow = length(p), ncol = length(dim))
...
for (i in 1:length(p)) {
  ...
  lsanrowl1_20NG[[i]] <- meanprelsacapvalue(dtm.training.lsanrowl1.svd.ud, dtm.testing.lsanrowl1.svd.ud, training_index, testing_index, metric = dis_metric, dim, retrieved_document_number = nrow_dtm_training, recall_value)
  lsanrowl1_euc_20NG[i,] <- lsanrowl1_20NG[[i]][[1]]
  lsanrowl1_cos_20NG[i,] <- lsanrowl1_20NG[[i]][[2]]
  lsanrowl1_dot_20NG[i,] <- lsanrowl1_20NG[[i]][[3]]
  }

lsanrowl1_20NG_p <- p
lsanrowl1_20NG_dim <- dim

save(lsanrowl1_20NG_dim, lsanrowl1_20NG_p, lsanrowl1_euc_20NG,lsanrowl1_cos_20NG, lsanrowl1_dot_20NG, file='C:\\Users\\qi000005\\OneDrive - Universiteit Utrecht\\qi000005\\paper 2\\20Newsgroups fold in IR\\created data\\nrowl1\\lsa\\p value\\meanprelsanrowl120NG.Rdata')
