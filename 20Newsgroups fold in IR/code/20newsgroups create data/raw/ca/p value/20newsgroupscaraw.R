...
caraw_euc_20NG <- caraw_cos_20NG <- caraw_dot_20NG <- matrix(NA, nrow = length(p), ncol = length(dim))
...
for (i in 1:length(p)) {
  ...
  caraw_20NG <- meanprelsacapvalue(dtm.training.caraw.svd.ud, dtm.testing.caraw.svd.ud, training_index, testing_index, metric = dis_metric, dim, retrieved_document_number = nrow_dtm_training, recall_value)
  caraw_euc_20NG[i,] <- caraw_20NG[[1]]
  caraw_cos_20NG[i,] <- caraw_20NG[[2]]
  caraw_dot_20NG[i,] <- caraw_20NG[[3]]
  }

caraw_20NG_p <- p
caraw_20NG_dim <- dim

save(caraw_20NG_dim, caraw_20NG_p, caraw_euc_20NG,caraw_cos_20NG, caraw_dot_20NG, file='C:\\Users\\qi000005\\OneDrive - Universiteit Utrecht\\qi000005\\paper 2\\20Newsgroups fold in IR\\created data\\raw\\ca\\p value\\meanprecaraw20NG.Rdata')
