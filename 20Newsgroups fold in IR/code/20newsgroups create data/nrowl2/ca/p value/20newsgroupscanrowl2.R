...
canrowl2_euc_20NG <- canrowl2_cos_20NG <- canrowl2_dot_20NG <- matrix(NA, nrow = length(p), ncol = length(dim))
...
for (i in 1:length(p)) {
  ...
  canrowl2_20NG <- meanprelsacapvalue(dtm.training.canrowl2.svd.ud, dtm.testing.canrowl2.svd.ud, training_index, testing_index, metric = dis_metric, dim, retrieved_document_number = nrow_dtm_training, recall_value)
  canrowl2_euc_20NG[i,] <- canrowl2_20NG[[1]]
  canrowl2_cos_20NG[i,] <- canrowl2_20NG[[2]]
  canrowl2_dot_20NG[i,] <- canrowl2_20NG[[3]]
  }

canrowl2_20NG_p <- p
canrowl2_20NG_dim <- dim

save(canrowl2_20NG_dim, canrowl2_20NG_p, canrowl2_euc_20NG,canrowl2_cos_20NG, canrowl2_dot_20NG, file='C:\\Users\\qi000005\\OneDrive - Universiteit Utrecht\\qi000005\\paper 2\\20Newsgroups fold in IR\\created data\\nrowl2\\ca\\p value\\meanprecanrowl220NG.Rdata')
