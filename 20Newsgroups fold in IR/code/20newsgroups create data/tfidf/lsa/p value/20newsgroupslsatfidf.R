...
lsatfidf_20NG <- list()
lsatfidf_euc_20NG <- lsatfidf_cos_20NG <- lsatfidf_dot_20NG <- matrix(NA, nrow = length(p), ncol = length(dim))
...
for (i in 1:length(p)) {
  ...
  dtm.testing.lsatfidf.svd.ud <- as.matrix(testing_dtm) %*% as.matrix(dtm.training.lsatfidf.svd$v)%*%as.matrix(diag((dtm.training.lsatfidf.svd$d)^(p[i]-1)))
  row.names(dtm.training.lsatfidf.svd.ud)  <- training_index
  lsatfidf_20NG[[i]] <- meanprelsacapvalue(dtm.training.lsatfidf.svd.ud, dtm.testing.lsatfidf.svd.ud, training_index, testing_index, metric = dis_metric, dim, retrieved_document_number = nrow_dtm_training, recall_value)
  lsatfidf_euc_20NG[i,] <- lsatfidf_20NG[[i]][[1]]
  lsatfidf_cos_20NG[i,] <- lsatfidf_20NG[[i]][[2]]
  lsatfidf_dot_20NG[i,] <- lsatfidf_20NG[[i]][[3]]
  }

lsatfidf_20NG_p <- p
lsatfidf_20NG_dim <- dim

save(lsatfidf_20NG_dim, lsatfidf_20NG_p, lsatfidf_euc_20NG,lsatfidf_cos_20NG, lsatfidf_dot_20NG, file='C:\\Users\\qi000005\\OneDrive - Universiteit Utrecht\\qi000005\\paper 2\\20Newsgroups fold in IR\\created data\\tfidf\\lsa\\p value\\meanprelsatfidf20NG.Rdata')
