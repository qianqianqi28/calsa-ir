...
canrowl1_euc_BN <- canrowl1_cos_BN <- canrowl1_dot_BN <- matrix(NA, nrow = length(p), ncol = length(dim))
...
for (i in 1:length(p)) {
  ...
  canrowl1_BN <- meanprelsacapvalue(dtm.training.canrowl1.svd.ud, dtm.testing.canrowl1.svd.ud, training_index, testing_index, metric = dis_metric, dim, retrieved_document_number = nrow_dtm_training, recall_value)
  canrowl1_euc_BN[i,] <- canrowl1_BN[[1]]
  canrowl1_cos_BN[i,] <- canrowl1_BN[[2]]
  canrowl1_dot_BN[i,] <- canrowl1_BN[[3]]
  }

canrowl1_BN_p <- p
canrowl1_BN_dim <- dim

save(canrowl1_BN_dim, canrowl1_BN_p, canrowl1_euc_BN,canrowl1_cos_BN, canrowl1_dot_BN, file='C:\\Users\\qi000005\\OneDrive - Universiteit Utrecht\\qi000005\\paper 2\\BBCNews data fold in IR\\created data\\nrowl1\\ca\\p value\\meanprecanrowl1BN.Rdata')
