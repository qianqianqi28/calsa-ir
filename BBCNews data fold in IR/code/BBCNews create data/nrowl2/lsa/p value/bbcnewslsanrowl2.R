...
lsanrowl2_BN <- list()
lsanrowl2_euc_BN <- lsanrowl2_cos_BN <- lsanrowl2_dot_BN <- matrix(NA, nrow = length(p), ncol = length(dim))
...
for (i in 1:length(p)) {
  ...
  lsanrowl2_BN[[i]] <- meanprelsacapvalue(dtm.training.lsanrowl2.svd.ud, dtm.testing.lsanrowl2.svd.ud, training_index, testing_index, metric = dis_metric, dim, retrieved_document_number = nrow_dtm_training, recall_value)
  lsanrowl2_euc_BN[i,] <- lsanrowl2_BN[[i]][[1]]
  lsanrowl2_cos_BN[i,] <- lsanrowl2_BN[[i]][[2]]
  lsanrowl2_dot_BN[i,] <- lsanrowl2_BN[[i]][[3]]
  }

lsanrowl2_BN_p <- p
lsanrowl2_BN_dim <- dim

save(lsanrowl2_BN_dim, lsanrowl2_BN_p, lsanrowl2_euc_BN,lsanrowl2_cos_BN, lsanrowl2_dot_BN, file='C:\\Users\\qi000005\\OneDrive - Universiteit Utrecht\\qi000005\\paper 2\\BBCNews data fold in IR\\created data\\nrowl2\\lsa\\p value\\meanprelsanrowl2BN.Rdata')
