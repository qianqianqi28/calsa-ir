...
lsatfidf_BN <- list()
lsatfidf_euc_BN <- lsatfidf_cos_BN <- lsatfidf_dot_BN <- matrix(NA, nrow = length(p), ncol = length(dim))
...
for (i in 1:length(p)) {
  ...
  lsatfidf_BN[[i]] <- meanprelsacapvalue(dtm.training.lsatfidf.svd.ud, dtm.testing.lsatfidf.svd.ud, training_index, testing_index, metric = dis_metric, dim, retrieved_document_number = nrow_dtm_training, recall_value)
  lsatfidf_euc_BN[i,] <- lsatfidf_BN[[i]][[1]]
  lsatfidf_cos_BN[i,] <- lsatfidf_BN[[i]][[2]]
  lsatfidf_dot_BN[i,] <- lsatfidf_BN[[i]][[3]]
  }

lsatfidf_BN_p <- p
lsatfidf_BN_dim <- dim

save(lsatfidf_BN_dim, lsatfidf_BN_p, lsatfidf_euc_BN,lsatfidf_cos_BN, lsatfidf_dot_BN, file='C:\\Users\\qi000005\\OneDrive - Universiteit Utrecht\\qi000005\\paper 2\\BBCNews data fold in IR\\created data\\tfidf\\lsa\\p value\\meanprelsatfidfBN.Rdata')
