...
catfidf_euc_BN <- catfidf_cos_BN <- catfidf_dot_BN <- matrix(NA, nrow = length(p), ncol = length(dim))
...
for (i in 1:length(p)) {
  ...
  catfidf_BN <- meanprelsacapvalue(dtm.training.catfidf.svd.ud, dtm.testing.catfidf.svd.ud, training_index, testing_index, metric = dis_metric, dim, retrieved_document_number = nrow_dtm_training, recall_value)
  catfidf_euc_BN[i,] <- catfidf_BN[[1]]
  catfidf_cos_BN[i,] <- catfidf_BN[[2]]
  catfidf_dot_BN[i,] <- catfidf_BN[[3]]
  }

catfidf_BN_p <- p
catfidf_BN_dim <- dim

save(catfidf_BN_dim, catfidf_BN_p, catfidf_euc_BN,catfidf_cos_BN, catfidf_dot_BN, file='C:\\Users\\qi000005\\OneDrive - Universiteit Utrecht\\qi000005\\paper 2\\BBCNews data fold in IR\\created data\\tfidf\\ca\\p value\\meanprecatfidfBN.Rdata')
