...
lsaraw_BN <- list()
lsaraw_euc_BN <- lsaraw_cos_BN <- lsaraw_dot_BN <- matrix(NA, nrow = length(p), ncol = length(dim))
...
for (i in 1:length(p)) {
  ...
  lsaraw_BN[[i]] <- meanprelsacapvalue(dtm.training.lsaraw.svd.ud, dtm.testing.lsaraw.svd.ud, training_index, testing_index, metric = dis_metric, dim, retrieved_document_number = nrow_dtm_training, recall_value)
  lsaraw_euc_BN[i,] <- lsaraw_BN[[i]][[1]]
  lsaraw_cos_BN[i,] <- lsaraw_BN[[i]][[2]]
  lsaraw_dot_BN[i,] <- lsaraw_BN[[i]][[3]]
  }

lsaraw_BN_p <- p
lsaraw_BN_dim <- dim

save(lsaraw_BN_dim, lsaraw_BN_p, lsaraw_euc_BN,lsaraw_cos_BN, lsaraw_dot_BN, file='C:\\Users\\qi000005\\OneDrive - Universiteit Utrecht\\qi000005\\paper 2\\BBCNews data fold in IR\\created data\\raw\\lsa\\p value\\meanprelsarawBN.Rdata')
