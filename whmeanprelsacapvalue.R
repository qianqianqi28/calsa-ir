...
whmeanprelsacapvalue <- function (dtm.training.svd.ud, dtm.testing.svd.ud, training_index, testing_index, metric = "euclidean", dimensionrange, retrieved_document_number = 20, recall_value) {
  ...
  for (k in (2:length(dimensionrange))){
    Euc_test_train[[k]] <- apply(dtm.training.svd.ud[, 1:dimensionrange[k]],1,function(y) apply(t(as.matrix(dtm.testing.svd.ud[, 1:dimensionrange[k]])),1,function(x,y)dist(rbind(x,y)),y))
  }
  ...
  for (k in (2:length(dimensionrange))){
    Cos_test_train[[k]] <- apply(dtm.training.svd.ud[, 1:dimensionrange[k]],1,function(y) apply(t(as.matrix(dtm.testing.svd.ud[, 1:dimensionrange[k]])),1,function(x,y)cosine(x,y),y))
  }

  #Dot
  
  for (k in (1:length(dimensionrange))){
    Dot_test_train[[k]] <- apply(as.matrix(dtm.training.svd.ud[, 1:dimensionrange[k]]),1,function(y) apply(t(as.matrix(dtm.testing.svd.ud[, 1:dimensionrange[k]])),1,function(x,y)(t(x) %*% y),y))
  }
  ...
  for(i in 1:nrow_testing){
    ...
    for (k in (1:length(dimensionrange))){
      ...
      Euc_query_train_one <- as.matrix(t(as.matrix(Euc_test_train[[k]]))[i,])
      ...
      Cos_query_train_one <- as.matrix(t(as.matrix(Cos_test_train[[k]]))[i,])
      ...
      Dot_query_train_one <- as.matrix(t(as.matrix(Dot_test_train[[k]]))[i,])
