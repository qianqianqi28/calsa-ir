...
for (n in loo.len){
  ...
  canrowl2_WH <- list()
  euc_canrowl2_WH <- cos_canrowl2_WH <- dot_canrowl2_WH <- matrix(NA, nrow = length(p), ncol = length(dim))
  # max_euc_canrowl2_WH <- max_cos_canrowl2_WH <- max_dot_canrowl2_WH <- matrix(NA, nrow = 1, ncol = length(p))
  ...
  for (i in 1:length(p)) {
    ...
    row.names(dtm.training.canrowl2.svd.ud)  <- training_index[[n]]
    
    
    canrowl2_WH[[i]] <- whmeanprelsacapvalue(dtm.training.canrowl2.svd.ud, dtm.testing.canrowl2.svd.ud, training_index[[n]], testing_index[[n]], metric = dis_metric, dim, retrieved_document_number = nrow_dtm_training, recall_value)
    euc_canrowl2_WH[i,] <- canrowl2_WH[[i]][[1]]
    cos_canrowl2_WH[i,] <- canrowl2_WH[[i]][[2]]
    dot_canrowl2_WH[i,] <- canrowl2_WH[[i]][[3]]
    # print(euc_canrowl2_WH[i,])
    # print(cos_canrowl2_WH[i,])
    # print(dot_canrowl2_WH[i,])
  }
  euc_sum_temp <- euc_sum_temp + euc_canrowl2_WH
  cos_sum_temp <- cos_sum_temp + cos_canrowl2_WH
  dot_sum_temp <- dot_sum_temp + dot_canrowl2_WH
}
canrowl2_euc_WH <- euc_sum_temp/length(testing_dtm)
canrowl2_cos_WH <- cos_sum_temp/length(testing_dtm)
canrowl2_dot_WH <- dot_sum_temp/length(testing_dtm)



canrowl2_WH_p <- p
canrowl2_WH_dim <- dim
save(canrowl2_WH_dim, canrowl2_WH_p, canrowl2_euc_WH,canrowl2_cos_WH, canrowl2_dot_WH, file='C:\\Users\\qi000005\\OneDrive - Universiteit Utrecht\\qi000005\\paper 2\\Wilhelmus data fold in IR\\created data\\nrowl2\\ca\\p value\\meanprecanrowl2WH.Rdata')
