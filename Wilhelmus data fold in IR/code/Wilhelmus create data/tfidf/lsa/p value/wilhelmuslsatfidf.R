...
for (n in loo.len){
  ...
  lsatfidf_WH <- list()
  euc_lsatfidf_WH <- cos_lsatfidf_WH <- dot_lsatfidf_WH <- matrix(NA, nrow = length(p), ncol = length(dim))
  # max_euc_lsatfidf_WH <- max_cos_lsatfidf_WH <- max_dot_lsatfidf_WH <- matrix(NA, nrow = 1, ncol = length(p))
  ...
  for (i in 1:length(p)) {
    ...
    lsatfidf_WH[[i]] <- whmeanprelsacapvalue(dtm.training.lsatfidf.svd.ud, dtm.testing.lsatfidf.svd.ud, training_index[[n]], testing_index[[n]], metric = dis_metric, dim, retrieved_document_number = nrow_dtm_training, recall_value)
    euc_lsatfidf_WH[i,] <- lsatfidf_WH[[i]][[1]]
    cos_lsatfidf_WH[i,] <- lsatfidf_WH[[i]][[2]]
    dot_lsatfidf_WH[i,] <- lsatfidf_WH[[i]][[3]]
    # print(euc_lsatfidf_WH[i,])
    # print(cos_lsatfidf_WH[i,])
    # print(dot_lsatfidf_WH[i,])
  }
  euc_sum_temp <- euc_sum_temp + euc_lsatfidf_WH
  cos_sum_temp <- cos_sum_temp + cos_lsatfidf_WH
  dot_sum_temp <- dot_sum_temp + dot_lsatfidf_WH
}
lsatfidf_euc_WH <- euc_sum_temp/length(testing_dtm)
lsatfidf_cos_WH <- cos_sum_temp/length(testing_dtm)
lsatfidf_dot_WH <- dot_sum_temp/length(testing_dtm)



lsatfidf_WH_p <- p
lsatfidf_WH_dim <- dim
save(lsatfidf_WH_dim, lsatfidf_WH_p, lsatfidf_euc_WH,lsatfidf_cos_WH, lsatfidf_dot_WH, file='C:\\Users\\qi000005\\OneDrive - Universiteit Utrecht\\qi000005\\paper 2\\Wilhelmus data fold in IR\\created data\\tfidf\\lsa\\p value\\meanprelsatfidfWH.Rdata')
