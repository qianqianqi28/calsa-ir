...
for (n in loo.len){
  ...
  lsanrowl1_WH <- list()
  euc_lsanrowl1_WH <- cos_lsanrowl1_WH <- dot_lsanrowl1_WH <- matrix(NA, nrow = length(p), ncol = length(dim))
  # max_euc_lsanrowl1_WH <- max_cos_lsanrowl1_WH <- max_dot_lsanrowl1_WH <- matrix(NA, nrow = 1, ncol = length(p))
  ...
  for (i in 1:length(p)) {
    ...
    lsanrowl1_WH[[i]] <- whmeanprelsacapvalue(dtm.training.lsanrowl1.svd.ud, dtm.testing.lsanrowl1.svd.ud, training_index[[n]], testing_index[[n]], metric = dis_metric, dim, retrieved_document_number = nrow_dtm_training, recall_value)
    euc_lsanrowl1_WH[i,] <- lsanrowl1_WH[[i]][[1]]
    cos_lsanrowl1_WH[i,] <- lsanrowl1_WH[[i]][[2]]
    dot_lsanrowl1_WH[i,] <- lsanrowl1_WH[[i]][[3]]
    # print(euc_lsanrowl1_WH[i,])
    # print(cos_lsanrowl1_WH[i,])
    # print(dot_lsanrowl1_WH[i,])
  }
  euc_sum_temp <- euc_sum_temp + euc_lsanrowl1_WH
  cos_sum_temp <- cos_sum_temp + cos_lsanrowl1_WH
  dot_sum_temp <- dot_sum_temp + dot_lsanrowl1_WH
}
lsanrowl1_euc_WH <- euc_sum_temp/length(testing_dtm)
lsanrowl1_cos_WH <- cos_sum_temp/length(testing_dtm)
lsanrowl1_dot_WH <- dot_sum_temp/length(testing_dtm)



lsanrowl1_WH_p <- p
lsanrowl1_WH_dim <- dim
save(lsanrowl1_WH_dim, lsanrowl1_WH_p, lsanrowl1_euc_WH,lsanrowl1_cos_WH, lsanrowl1_dot_WH, file='C:\\Users\\qi000005\\OneDrive - Universiteit Utrecht\\qi000005\\paper 2\\Wilhelmus data fold in IR\\created data\\nrowl1\\lsa\\p value\\meanprelsanrowl1WH.Rdata')
