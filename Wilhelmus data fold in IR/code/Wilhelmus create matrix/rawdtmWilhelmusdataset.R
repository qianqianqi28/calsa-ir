#Same as the calsa-tc/Wilhelmus data fold in/code/Wilhelmus create matrix/rawdtmWilhelmusdataset.R


#Copy from the calsa-tc/Wilhelmus data fold in/code/Wilhelmus create matrix/rawdtmWilhelmusdataset.R. In order to run the code in the same folder, we modify the path of data

#This code is for songs from six authors using LOOCV. Note each time 185 documents from training set form a raw document-term matrix of size 185*300 and the single document of validation set is not included in the matrix.
...
for (i in 1:length(include_authors)) {
  path <- paste0("C:/Users/qi000005/OneDrive - Universiteit Utrecht/qi000005/paper 2/Wilhelmus data fold in IR/data", "/", include_authors[i], "/")
...
save(training_dtm, testing_dtm, training_index, testing_index, file = "C:\\Users\\qi000005\\OneDrive - Universiteit Utrecht\\qi000005\\paper 2\\Wilhelmus data fold in IR\\created data\\created matrix\\rawdtmWilhelmusdataset.Rdata")
