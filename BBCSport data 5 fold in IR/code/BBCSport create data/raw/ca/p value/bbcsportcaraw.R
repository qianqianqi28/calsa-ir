...
for (n in loo.len){
  ...
  caraw_BS <- list()
  euc_caraw_BS <- cos_caraw_BS <- dot_caraw_BS <- matrix(NA, nrow = length(p), ncol = length(dim))
  # max_euc_caraw_BS <- max_cos_caraw_BS <- max_dot_caraw_BS <- matrix(NA, nrow = 1, ncol = length(p))
  ...
  for (i in 1:length(p)) {
    ...
    caraw_BS[[i]] <- meanprelsacapvalue(dtm.training.caraw.svd.ud, dtm.testing.caraw.svd.ud, training_index[[n]], testing_index[[n]], metric = dis_metric, dim, retrieved_document_number = nrow_dtm_training, recall_value)
    euc_caraw_BS[i,] <- caraw_BS[[i]][[1]]
    cos_caraw_BS[i,] <- caraw_BS[[i]][[2]]
    dot_caraw_BS[i,] <- caraw_BS[[i]][[3]]
    # print(euc_caraw_BS[i,])
    # print(cos_caraw_BS[i,])
    # print(dot_caraw_BS[i,])
    }
  euc_sum_temp <- euc_sum_temp + euc_caraw_BS
  cos_sum_temp <- cos_sum_temp + cos_caraw_BS
  dot_sum_temp <- dot_sum_temp + dot_caraw_BS
}
caraw_euc_BS <- euc_sum_temp/length(testing_dtm)
caraw_cos_BS <- cos_sum_temp/length(testing_dtm)
caraw_dot_BS <- dot_sum_temp/length(testing_dtm)



caraw_BS_p <- p
caraw_BS_dim <- dim
save(caraw_BS_dim, caraw_BS_p, caraw_euc_BS,caraw_cos_BS, caraw_dot_BS, file='C:\\Users\\qi000005\\OneDrive - Universiteit Utrecht\\qi000005\\paper 2\\BBCSport data 5 fold in IR\\created data\\raw\\ca\\p value\\meanprecarawBS.Rdata')
