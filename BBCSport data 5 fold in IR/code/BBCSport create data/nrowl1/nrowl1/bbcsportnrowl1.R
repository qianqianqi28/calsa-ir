...
for (n in loo.len){
  ...
  euc_nrowl1_BS <- cos_nrowl1_BS <- dot_nrowl1_BS <- matrix(NA, nrow = length(p), ncol = 1)

  dis_metric <- "all"
  

  row.names(training_dtm[[n]])  <- training_index[[n]]
  nrowl1_BS <- meanprerawnrowtfidfpvalue(training_dtm[[n]], testing_dtm[[n]], training_index[[n]], testing_index[[n]], metric = dis_metric, retrieved_document_number = nrow_dtm_training, recall_value)
  euc_nrowl1_BS <- nrowl1_BS[[1]]
  cos_nrowl1_BS <- nrowl1_BS[[2]]
  dot_nrowl1_BS <- nrowl1_BS[[3]]
  # print(euc_nrowl1_BS[i,])
  # print(cos_nrowl1_BS[i,])
  # print(dot_nrowl1_BS[i,])
  
  euc_sum_temp <- euc_sum_temp + euc_nrowl1_BS
  cos_sum_temp <- cos_sum_temp + cos_nrowl1_BS
  dot_sum_temp <- dot_sum_temp + dot_nrowl1_BS
}
nrowl1_euc_BS <- euc_sum_temp/length(testing_dtm)
nrowl1_cos_BS <- cos_sum_temp/length(testing_dtm)
nrowl1_dot_BS <- dot_sum_temp/length(testing_dtm)



nrowl1_BS_p <- p
save(nrowl1_BS_p, nrowl1_euc_BS,nrowl1_cos_BS, nrowl1_dot_BS, file='C:\\Users\\qi000005\\OneDrive - Universiteit Utrecht\\qi000005\\paper 2\\BBCSport data 5 fold in IR\\created data\\nrowl1\\nrowl1\\meanprenrowl1BS.Rdata')
