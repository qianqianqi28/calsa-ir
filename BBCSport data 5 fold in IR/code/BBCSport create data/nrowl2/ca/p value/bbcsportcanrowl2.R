...
for (n in loo.len){
  ...
  canrowl2_BS <- list()
  euc_canrowl2_BS <- cos_canrowl2_BS <- dot_canrowl2_BS <- matrix(NA, nrow = length(p), ncol = length(dim))
  # max_euc_canrowl2_BS <- max_cos_canrowl2_BS <- max_dot_canrowl2_BS <- matrix(NA, nrow = 1, ncol = length(p))
  ...
  for (i in 1:length(p)) {
    ...
    canrowl2_BS[[i]] <- meanprelsacapvalue(dtm.training.canrowl2.svd.ud, dtm.testing.canrowl2.svd.ud, training_index[[n]], testing_index[[n]], metric = dis_metric, dim, retrieved_document_number = nrow_dtm_training, recall_value)
    euc_canrowl2_BS[i,] <- canrowl2_BS[[i]][[1]]
    cos_canrowl2_BS[i,] <- canrowl2_BS[[i]][[2]]
    dot_canrowl2_BS[i,] <- canrowl2_BS[[i]][[3]]
    # print(euc_canrowl2_BS[i,])
    # print(cos_canrowl2_BS[i,])
    # print(dot_canrowl2_BS[i,])
    }
  euc_sum_temp <- euc_sum_temp + euc_canrowl2_BS
  cos_sum_temp <- cos_sum_temp + cos_canrowl2_BS
  dot_sum_temp <- dot_sum_temp + dot_canrowl2_BS
}
canrowl2_euc_BS <- euc_sum_temp/length(testing_dtm)
canrowl2_cos_BS <- cos_sum_temp/length(testing_dtm)
canrowl2_dot_BS <- dot_sum_temp/length(testing_dtm)



canrowl2_BS_p <- p
canrowl2_BS_dim <- dim
save(canrowl2_BS_dim, canrowl2_BS_p, canrowl2_euc_BS,canrowl2_cos_BS, canrowl2_dot_BS, file='C:\\Users\\qi000005\\OneDrive - Universiteit Utrecht\\qi000005\\paper 2\\BBCSport data 5 fold in IR\\created data\\nrowl2\\ca\\p value\\meanprecanrowl2BS.Rdata')
