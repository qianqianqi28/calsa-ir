...
for (n in loo.len){
  ...
  euc_tfidf_BS <- cos_tfidf_BS <- dot_tfidf_BS <- matrix(NA, nrow = length(p), ncol = 1)

  dis_metric <- "all"
  

  row.names(training_dtm[[n]])  <- training_index[[n]]
  tfidf_BS <- meanprerawnrowtfidfpvalue(training_dtm[[n]], testing_dtm[[n]], training_index[[n]], testing_index[[n]], metric = dis_metric, retrieved_document_number = nrow_dtm_training, recall_value)
  euc_tfidf_BS <- tfidf_BS[[1]]
  cos_tfidf_BS <- tfidf_BS[[2]]
  dot_tfidf_BS <- tfidf_BS[[3]]
  # print(euc_tfidf_BS[i,])
  # print(cos_tfidf_BS[i,])
  # print(dot_tfidf_BS[i,])
  
  euc_sum_temp <- euc_sum_temp + euc_tfidf_BS
  cos_sum_temp <- cos_sum_temp + cos_tfidf_BS
  dot_sum_temp <- dot_sum_temp + dot_tfidf_BS
}
tfidf_euc_BS <- euc_sum_temp/length(testing_dtm)
tfidf_cos_BS <- cos_sum_temp/length(testing_dtm)
tfidf_dot_BS <- dot_sum_temp/length(testing_dtm)



tfidf_BS_p <- p
save(tfidf_BS_p, tfidf_euc_BS,tfidf_cos_BS, tfidf_dot_BS, file='C:\\Users\\qi000005\\OneDrive - Universiteit Utrecht\\qi000005\\paper 2\\BBCSport data 5 fold in IR\\created data\\tfidf\\tfidf\\meanpretfidfBS.Rdata')
