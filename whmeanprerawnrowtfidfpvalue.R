...
whmeanprerawnrowtfidfpvalue <- function (dtm.training.svd.ud, dtm.testing.svd.ud, training_index, testing_index, metric = "euclidean", retrieved_document_number = 20, recall_value) {
  ...
  for(i in 1:nrow_testing){
    ...
    Euc_query_train_one <- as.matrix(t(as.matrix(Euc_test_train))[i,])
    ...
    Cos_query_train_one <- as.matrix(t(as.matrix(Cos_test_train))[i,])
    ...
    Dot_query_train_one <- as.matrix(t(as.matrix(Dot_test_train))[i,])
